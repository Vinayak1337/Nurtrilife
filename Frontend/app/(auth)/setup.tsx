...
export default function SetupScreen() {
	...
	const [selectedPreset, setSelectedPreset] =
		useState<MacroPresetKey>('balanced');
	...
	function validateCalorieStep() {
		const custom = Number.parseInt(customCalories, 10);
		if (
			customCalories &&
			(Number.isNaN(custom) || custom < 1000 || custom > 5000)
		) {
			return false;
		}
		if (customCalories && !Number.isNaN(custom)) {
			setCalorieGoal(custom);
		}
		return true;
	}

	function applyHydrationStep() {
		const custom = Number.parseInt(customWater, 10);
		if (customWater && !Number.isNaN(custom)) {
			setWaterGoal(custom);
		}
	}
	...
	function getNextButtonLabel() {
		...
		if (step === STEP_LABELS.length - 1) return '✓ Complete Setup';
		return 'Continue';
	}

	return (
		<LinearGradient colors={['#0A0F1E', '#111827']} style={styles.gradient}>
					...
					<Animated.View entering={FadeInDown.delay(100)} style={styles.header}>
						<View style={styles.stepIndicator}>
							{STEP_LABELS.map((label, i) => (
								<View key={i + 'step-label' + label} style={styles.stepItem}>
									<View
										style={[
											styles.stepDot,
											{
												...
												borderColor:
													i === step ? Palette.primary : 'transparent',
									...
									{i < STEP_LABELS.length - 1 && (
										<View
											style={[
												...
												{
													backgroundColor:
														i < step ? Palette.primary : 'rgba(255,255,255,0.1)'
												}
											]}
										/>
									)}
						...
						{step === 1 && (
							<Animated.View
									...
									{CALORIE_PRESETS.map(cal => (
										<Pressable
											...
											style={[
												...
												{
													backgroundColor:
														calorieGoal === cal && !customCalories
															? Palette.primary
								...
								<Input
									...
									onChangeText={t => {
										setCustomCalories(t);
										if (t) setCalorieGoal(Number.parseInt(t) || 2000);
									}}
										...
										return (
											<Pressable
												...
												<View style={styles.macroPresetTop}>
													<AppText
														variant='subheading'
														color={isSelected ? Palette.primary : '#FFF'}>
														{preset.label}
													</AppText>
													{isSelected && (
														<View
						...
						{step === 3 && (
							<Animated.View
								...
								<Input
									...
									onChangeText={t => {
										setCustomWater(t);
										if (t) setWaterGoal(Number.parseInt(t) || 2500);
									}}
									keyboardType='numeric'
									...
									{HEALTH_FOCUS_OPTIONS.map(option => {
										...
										return (
											<Pressable
												...
												<AppText style={styles.focusEmoji}>
													{option.emoji}
												</AppText>
												<AppText
													variant='label'
