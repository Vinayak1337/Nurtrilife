...
export default function VerifyScreen() {
  const { flow, email } = useLocalSearchParams<{ flow: 'sign-in' | 'sign-up'; email: string }>();
  ...
  function handleChange(text: string, index: number) {
    ...
    if (text.length === CODE_LENGTH) {
      const chars = text.split('').filter((c) => /\d/.test(c)).slice(0, CODE_LENGTH);
    ...
    try {
      if (flow === 'sign-up' && signUp) {
        ...
        if (result.createdSessionId && setSignUpActive) {
          ...
          setError('Account setup incomplete — check your Clerk dashboard configuration.');
        }
        ...
        >
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(500)} style={styles.header}>
            <View style={styles.iconWrap}>
              <IconSymbol name="checkmark.circle.fill" size={32} color={Palette.primary} />
            </View>
            <AppText variant="heading" color="#FFFFFF">
              Verify Email
            </AppText>
            <AppText variant="body" color="rgba(255,255,255,0.5)" style={{ textAlign: 'center' }}>
              Enter the 6-digit code sent to{'\n'}
              <AppText variant="body" color={Palette.primary}>{email}</AppText>
            </AppText>
          </Animated.View>

          <Animated.View entering={FadeInDown.delay(200).duration(500)} style={styles.codeRow}>
            {code.map((digit, i) => (
              <TextInput
                key={i}
          ...
          {error ? (
            <Animated.View entering={FadeInDown.duration(300)}>
              <AppText variant="bodySmall" color={Colors.light.accent} style={styles.error}>
                {error}
              </AppText>
            </Animated.View>
          ) : null}
