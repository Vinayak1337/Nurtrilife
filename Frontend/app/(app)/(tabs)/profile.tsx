...
export default function ProfileScreen() {
  ...
  const stats = useMemo<Stats>(() => {
    ...
    if (totalMeals === 0) return { totalMeals: 0, activeDays: 0, avgCalories: 0 };
    ...
    const avgCalories = activeDays > 0
      ? Math.round([...calsByDay.values()].reduce((s, c) => s + c, 0) / activeDays)
      : 0;
  ...
  function saveGoal() {
    ...
    if (isNaN(value) || value < min || value > max) {
      Alert.alert('Invalid Value', `Please enter a value between ${min} and ${max}.`);
      return;
    }
