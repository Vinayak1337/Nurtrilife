...
import { Colors, Palette, Spacing, FontFamily } from '@/constants/theme';
...
export default function InsightsScreen() {
  ...
  const { weeklyData, healthScore, deficiencyAlerts, isLoading, error } = useAppSelector(
    (s) => s.insights,
  );
  ...
  const { barData, lineLabels, proteinData, carbsData, fatsData } = useMemo(() => {
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
        <View style={styles.headerSection}>
          <AppText variant="heading" color={theme.text}>Insights</AppText>
        </View>
        <EmptyState
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={handleRefresh} tintColor={Palette.primary} />
        }
      >
        <View style={styles.headerSection}>
          <AppText variant="heading" color={theme.text}>Insights</AppText>
          ...
          <Animated.View entering={FadeInDown.delay(100).duration(500)}>
            <Card style={styles.scoreCard}>
              ...
              <View style={styles.breakdownRow}>
                <BreakdownItem label="Calories" value={healthScore.breakdown.calorieScore} max={30} color={Palette.secondary} theme={theme} />
                <BreakdownItem label="Protein" value={healthScore.breakdown.proteinScore} max={20} color={Palette.protein} theme={theme} />
                <BreakdownItem label="Balance" value={healthScore.breakdown.macroBalance} max={20} color={Palette.primary} theme={theme} />
              </View>
            </Card>
          </Animated.View>
        )}
            ...
            <View style={styles.streakRow}>
              <View style={styles.streakInfo}>
                <View style={styles.streakHeader}>
                  <IconSymbol name="flame.fill" size={22} color={Palette.secondary} />
              ...
              <View style={[styles.streakBadge, { backgroundColor: `${Palette.secondary}15` }]}>
                <IconSymbol name="flame.fill" size={36} color={Palette.secondary} />
              </View>
            </View>
          </Card>
        </Animated.View>
        ...
        {deficiencyAlerts.length > 0 && (
          <Animated.View entering={FadeInDown.delay(500).duration(500)}>
            ...
            {deficiencyAlerts.map((alert, i) => (
              <DeficiencyAlertCard key={i} alert={alert} />
            ))}
          </Animated.View>
        )}
      </ScrollView>
  ...
  return (
    <View style={bdStyles.item}>
      ...
      <View style={[bdStyles.track, { backgroundColor: `${color}20` }]}>
        <View style={[bdStyles.fill, { width: `${pct * 100}%`, backgroundColor: color }]} />
      </View>
