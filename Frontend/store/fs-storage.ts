...
export const fsStorage = {
  async setItem(key: string, value: string): Promise<void> {
    try {
      ...
      // Silently ignore — happens in Expo Go running as @anonymous where the
      // scoped document directory isn't writable. Persisted state will simply
      // not survive app restarts in that environment (dev-build fixes this).
    }
  },
