...
function Confetti({
	delay,
	color
}: Readonly<{ delay: number; color: string }>) {
	...
	useEffect(() => {
		translateY.value = withDelay(
			delay,
			withTiming(SCREEN_HEIGHT + 50, { duration: 2500 })
		);
...
export function CelebrationOverlay({
	badgeId,
	onDismiss
}: Readonly<CelebrationOverlayProps>) {
	...
	return (
		<Animated.View
			...
			<Animated.View style={[styles.card, cardStyle]}>
				...
				<View
					style={[styles.badgeIcon, { backgroundColor: `${badge.color}18` }]}>
