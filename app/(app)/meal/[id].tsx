...
import { getMealById } from '@/services/database.service';
...
export default function MealDetailScreen() {
  ...
  if (!meal) {
    return (
      <View style={[styles.centered, { backgroundColor: theme.background }]}>
        <AppText variant="heading" color={theme.text} align="center">Meal not found</AppText>
        <Button variant="ghost" size="md" onPress={() => router.back()}>Go Back</Button>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        <View style={styles.content}>
          ...
          <Animated.View entering={FadeInDown.delay(150)}>
            ...
            <View style={styles.metaRow}>
              <View style={[styles.typeBadge, { backgroundColor: `${mealMeta.color}20` }]}>
                <AppText variant="caption" color={mealMeta.color} weight="semiBold">
            ...
            {meal.description ? (
              <AppText variant="body" color={theme.textTertiary} style={{ marginTop: Spacing.sm }}>
                {meal.description}
              </AppText>
            ) : null}
          </Animated.View>
          ...
          {/* Big calorie */}
          <Animated.View entering={FadeInDown.delay(200)} style={styles.calorieBlock}>
            <AppText style={{ fontFamily: 'SpaceGrotesk_700Bold', fontSize: 52, color: Palette.secondary }}>
              {Math.round(meal.calories)}
            </AppText>
            <AppText variant="subheading" color={theme.textTertiary}>Calories</AppText>
          </Animated.View>

          {/* Macro cards */}
          <Animated.View entering={FadeInDown.delay(280)} style={styles.macroCards}>
            {(['protein', 'carbs', 'fats'] as const).map((key) => {
              const meta = NUTRIENT_META[key];
              const value = meal[key];
              return (
                <Card key={key} style={[styles.macroCard, { borderTopWidth: 3, borderTopColor: meta.color }]}>
                  <AppText style={{ fontSize: 22 }} align="center">{meta.icon}</AppText>
      ...
      {/* Delete button */}
      <Animated.View entering={FadeInDown.delay(500)} style={[styles.footer, { backgroundColor: theme.background }]}>
