...
import { getMealsByDate, getMealsByDateRange } from '@/services/database.service';
...
export default function CalendarScreen() {
  ...
  const [monthMeals, setMonthMeals] = useState<DayMeals>({});
  const [selectedMeals, setSelectedMeals] = useState<Meal[]>([]);
  const [loading, setLoading] = useState(false);

  const fetchMonthMeals = useCallback(async (month: string) => {
    setLoading(true);
    try {
      const start = dayjs(month + '-01').startOf('month').format('YYYY-MM-DD');
      const end = dayjs(month + '-01').endOf('month').format('YYYY-MM-DD');
      const meals = await getMealsByDateRange(start, end);
  ...
  const selectedDayMeals = selectedMeals;
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(250)}>
          ...
          {selectedDayMeals.length === 0 && !loading ? (
